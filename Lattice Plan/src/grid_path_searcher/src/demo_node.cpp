...
double _time_interval     = 1.25;   // 前向积分的时候用的
int    _time_step         = 50;

Homeworktool * _homework_tool     = new Homeworktool();
// 前三层指针是把地图看成是 state_space, 最后一层指针就指向点本身 
...
void SLP(const Vector3d start_pt, const Vector3d start_velocity, const Vector3d target_pt)
{
    
}

void GetNearestLatticeNode(Vector3d _origin_pt)
{
...
void trajectoryLibrary(const Vector3d start_pt, const Vector3d start_velocity, const Vector3d target_pt)
{
    ...
    // 关于为什么要采用 _discretize_step + 1 的方式，是因为我们需要访问 索引为0-_discretize_step 即 长度为（_discretize_step + 1）的内存，因此在new申请时
    // 应该使用 _discretize_step + 1, 而不是 _discretize_step
            ...
            for(int k=0; k <= _discretize_step; k++){   //acc_input_az
                ...
                for(int step=0 ; step<=_time_step ; step ++) {
                    ...
                    */
                    ...
                    vel = vel + (acc_input * delta_time);  // v = v + at;
                    pos = pos + vel * delta_time + 0.5*(acc_input * pow(delta_time, 2.0)); // p = vt + 1/2 * a * t^2
